...
let activeDays = []; // ✅ قائمة الأيام المفعّلة
...
function initFirebaseData() {
    ...
    db.collection("settings").doc("global_status").onSnapshot(doc => {
        if(doc.exists) {
            const data = doc.data();
            // ✅ دعم النظام الجديد (activeDays) والقديم (currentDay+status)
            if (Array.isArray(data.activeDays)) {
                activeDays = data.activeDays;
                adminDay = activeDays.length > 0 ? activeDays[activeDays.length - 1] : 1;
                adminStatus = activeDays.length > 0 ? 'active' : 'closed';
            } else {
                adminDay = data.currentDay || 1;
                adminStatus = data.status || "closed";
                activeDays = (adminStatus === 'active') ? [adminDay] : [];
            }
        }
        updateLogs();
    });
...
function renderMap() {
    ...
    for (let i = 1; i <= 30; i++) {
        ...
        let isActive = activeDays.includes(i);
        if (isPlayed) {
