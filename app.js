...
window.openSpinWheel = function() {
    ...
    setTimeout(() => {
        ...
        // لو لعب قبل — ارسم الجنزير فوراً
        const wd = JSON.parse(localStorage.getItem('wheelData_' + user.id)) || {};
        if (wd.spun) {
            setTimeout(() => { if (typeof window.drawLock === 'function') window.drawLock(); }, 80);
        }
    }, 60);

    let wheelData = JSON.parse(localStorage.getItem('wheelData_' + user.id)) || { spun: false };
    let controls  = document.getElementById('wheel-controls');

    if (wheelData.spun) {
        // مقفول — مفيش زرار
        controls.innerHTML = `
            <p style="font-family:'Cairo',sans-serif;color:#6b7280;font-size:13px;
               text-align:center;margin-top:16px;padding:0 8px;">
               لفتها بالفعل 🔒<br>
               <span style="font-size:11px;color:#4b5563;">انتظر التحديث القادم من الأدمن</span>
            </p>`;
    } else {
        // مفتوح — زرار مباشرة بدون شرط
