...
window.activeDays = []; // الأيام المفعّلة
...
function initFirebaseData() {
    ...
    db.collection("users").doc(user.id).onSnapshot(doc => {
        if(doc.exists) {
            ...
            // ✅ تحديث user من Firebase عشان group و status دايما محدّثة
            user.group  = d.group  || user.group  || "";
            ...
            // ✅ تحقق من التأهل لو أول مرة
            if (typeof checkQualifyStatus === 'function') {
                checkQualifyStatus({...d, id: user.id});
            }
        }
    });
    ...
    db.collection("settings").doc("global_status").onSnapshot(doc => {
        if(doc.exists) {
            ...
            if (Array.isArray(data.activeDays) && data.activeDays.length > 0) {
                window.activeDays = data.activeDays;
                adminDay = window.activeDays[window.activeDays.length - 1];
                adminStatus = 'active';
            } else {
                adminDay = data.currentDay || 1;
                adminStatus = data.status || "closed";
                window.activeDays = (adminStatus === 'active') ? [adminDay] : [];
            }
        }
        updateLogs();
    });
...
function renderMap() {
    ...
    for (let i = 1; i <= 30; i++) {
        ...
        let isActive = window.activeDays.includes(i);
        if (isPlayed) {
...
function fetchLeaderboard() {
    ...
    const isQualified = user.status === 'qualified' || user.group === 'النهائي';
    const isEliminated = user.status === 'eliminated' || user.group === 'مقصيين';
    ...
    if (isQualified) {
        // ✅ متأهل — يشوف زملاءه في النهائي
        if (titleEl)    { titleEl.textContent = '🏆 ترتيب المتأهلين للنهائي'; titleEl.style.color = '#4ade80'; }
        if (subtitleEl) { subtitleEl.textContent = 'أنت في مجموعة النهائي ⭐'; subtitleEl.style.color = '#4ade80'; }
        ...
        db.collection('users').where('group', '==', 'النهائي').get().then(snap => {
            ...
            list.forEach((u, i) => {
                ...
                html += `<div style="display:flex;align-items:center;justify-content:space-between;
                    ...
                    background:${isMe ? 'rgba(74,222,128,0.12)' : 'rgba(31,41,55,0.5)'};
                    border:1px solid ${isMe ? 'rgba(74,222,128,0.35)' : 'rgba(55,65,81,0.5)'};">
                    <div style="display:flex;align-items:center;gap:10px;">
                        <span style="font-size:${rank<=3?'20px':'14px'};font-weight:900;color:${rank<=3?'#fbbf24':'#6b7280'};min-width:26px;text-align:center;">${medal}</span>
                        <span style="font-weight:900;font-size:14px;color:${isMe?'#4ade80':'white'};">${u.name || 'بطل'}${isMe ? ' 👈' : ''}</span>
                    </div>
                    <span style="font-weight:900;font-size:18px;color:#fbbf24;">${u.score || 0}</span>
                </div>`;
            });
            document.getElementById('group-list').innerHTML = html || '<p style="color:#6b7280;text-align:center;padding:20px;font-size:13px;">لا يوجد متأهلون بعد</p>';
        });

    } else if (isEliminated) {
        // ✅ مقصي — يشوف نفسه بس
        if (titleEl)    { titleEl.textContent = '🏅 نتيجتك'; titleEl.style.color = '#f87171'; }
        if (subtitleEl) { subtitleEl.textContent = 'أنت تلعب بشكل فردي'; subtitleEl.style.color = '#f87171'; }

        db.collection('users').doc(user.id).get().then(doc => {
            const d = doc.data();
            const html = `<div style="display:flex;align-items:center;justify-content:space-between;
                padding:14px;border-radius:12px;
                background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.25);">
                <div style="display:flex;align-items:center;gap:10px;">
                    <span style="font-size:20px;">🎯</span>
                    <span style="font-weight:900;font-size:15px;color:white;">${d.name || 'بطل'} <span style="color:#f87171;font-size:11px;">(أنت)</span></span>
                </div>
                <span style="font-weight:900;font-size:20px;color:#fbbf24;">${d.score || 0}</span>
            </div>
            <p style="text-align:center;color:#6b7280;font-size:11px;margin-top:12px;">استمر في اللعب وجمع النقاط 💪</p>`;
            document.getElementById('group-list').innerHTML = html;
        });

    } else {
        // ✅ عادي — يشوف مجموعته
        if (titleEl)    { titleEl.textContent = '🏅 ترتيب مجموعتك'; titleEl.style.color = 'white'; }
        if (subtitleEl) { subtitleEl.textContent = user.group || ''; subtitleEl.style.color = '#9ca3af'; }
            ...
            list.forEach((u, i) => {
                ...
                html += `<div class="flex items-center justify-between p-3 bg-gray-800/50 rounded-xl mb-2 border border-gray-700">
                    <div class="flex items-center gap-3">
                        <span class="w-6 text-center font-black ${rank <= 3 ? 'text-yellow-500 text-xl' : 'text-gray-400'}">${rank}</span>
                        <span class="font-bold ${isMe ? 'text-yellow-400' : 'text-white'}">${u.name || 'بطل'}</span>
                    </div>
...
window.openSpinWheel = function() {
    ...
    setTimeout(() => {
        if (typeof drawWheel === 'function') drawWheel(window._wheelRot || 0);
    }, 60);

    let wheelData = JSON.parse(localStorage.getItem('wheelData_' + user.id)) || { streak: 0, used: false };
    let controls  = document.getElementById('wheel-controls');

    // ✅ لو استخدمها قبل كده — شاشة القفل المتحركة
    if (wheelData.used) {
        // احسب الوقت المتبقي للأسبوع القادم (7 أيام من آخر استخدام)
        const lastUsed  = wheelData.lastUsed ? new Date(wheelData.lastUsed) : new Date();
        const nextUnlock = new Date(lastUsed);
        nextUnlock.setDate(nextUnlock.getDate() + 3); // فتحها كل 3 أيام
        nextUnlock.setHours(0, 0, 0, 0);

        controls.innerHTML = `
            <div style="text-align:center;margin-top:8px;">
                <div id="lock-icon" style="font-size:52px;animation:lockBounce 1s ease-in-out infinite alternate;display:inline-block;">🔒</div>
                <p style="font-family:'Cairo',sans-serif;color:#9ca3af;font-size:13px;font-weight:700;margin:10px 0 6px;">
                    لفتها بالفعل! تفتح تاني بعد اكتمال 3 أيام جديدة
                </p>
                <div style="background:rgba(0,0,0,0.4);border-radius:10px;padding:10px;margin-bottom:4px;">
                    <p style="color:#6b7280;font-size:11px;margin-bottom:4px;">الوقت المتبقي للفتح</p>
                    <p id="wheel-timer-inside" style="color:#fbbf24;font-weight:900;font-size:22px;font-family:monospace;letter-spacing:3px;">--:--:--</p>
                </div>
                <p style="font-family:'Cairo',sans-serif;color:#4b5563;font-size:11px;">ادخل كل يوم عشان تحافظ على الـ streak 🔥</p>
            </div>`;
    ...
    } else if (wheelData.streak >= 3) {
        // ✅ مستحق اللعب
        controls.innerHTML = `<button id="spin-btn" onclick="doCanvasSpin()"
            style="width:100%;background:linear-gradient(135deg,#d97706,#b45309);color:white;border:none;
            border-radius:14px;padding:15px;font-family:'Cairo',sans-serif;font-size:17px;font-weight:900;
            cursor:pointer;box-shadow:0 4px 20px rgba(217,119,6,0.5);margin-top:8px;-webkit-tap-highlight-color:transparent;">
            🎰 العب دلوقتي!
        </button>`;
    } else {
        // ✅ لسه مكملش 3 أيام
        controls.innerHTML = `
            <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:14px;padding:16px;margin-top:8px;">
                <p style="font-family:'Cairo',sans-serif;color:#d1d5db;font-weight:700;margin-bottom:10px;font-size:13px;">
                    محتاج تدخل <span style="color:#fbbf24;font-size:16px;font-weight:900;">${3 - wheelData.streak}</span> أيام كمان ورا بعض!
                </p>
                <div style="background:rgba(0,0,0,0.4);border-radius:10px;padding:10px;">
                    <p style="color:#6b7280;font-size:11px;margin-bottom:4px;">الوقت المتبقي لليوم التالي</p>
                    <p id="wheel-timer-inside" style="color:#fbbf24;font-weight:900;font-size:24px;font-family:monospace;letter-spacing:3px;">--:--:--</p>
                </div>
            </div>`;
        const iv = setInterval(() => {
            const el = document.getElementById('wheel-timer-inside');
            ...
            el.textContent = String(Math.floor(diff/3600000)).padStart(2,'0') + ':' +
...
window.spinWheel = function() {
    ...
    setTimeout(() => {
        ...
        wheelData.streak = 0;
        wheelData.used = true;
        wheelData.lastUsed = new Date().toISOString();
        localStorage.setItem('wheelData_' + user.id, JSON.stringify(wheelData));
